...
static int fdevent_select_poll(fdevents* ev, int timeout_ms){
	...
	return select(ev->select_max_fd + 1, ev->select_read, ev->select_write, ev->select_error, &tval);
}
...
static int fdevent_select_event_next_fdndx(fdevents* ev, size_t ndx){
	int i;
	i = (ndx < 0) ? 0 : ndx + 1;

	for (i; i < ev->select_max_fd + 1; i++){
